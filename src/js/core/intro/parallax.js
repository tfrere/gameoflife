...
;((($, window, document, undefined) => {
  ...
  class Plugin {
    ...
    onAnimationFrame() {
      this.updateBounds();
      const dx = this.ix - this.cx;
      const dy = this.iy - this.cy;
      ...
      for (let i = 0, l = this.$layers.length; i < l; i++) {
        ...
        const xOffset = this.vx * depth * (this.invertX ? -1 : 1);
        const yOffset = this.vy * depth * (this.invertY ? -1 : 1);
    ...
    onDeviceOrientation(event) {
      ...
      if (!this.desktop && event.beta !== null && event.gamma !== null) {
        ...
        // Extract Rotation
        const x = (event.beta  || 0) / MAGIC_NUMBER; //  -90 :: 90
        const y = (event.gamma || 0) / MAGIC_NUMBER; // -180 :: 180
